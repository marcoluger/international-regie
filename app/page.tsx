                    ...
                    return (
                      <div key={p.id} className="border border-slate-200 rounded-xl p-3 bg-gray-50 flex justify-between items-start gap-2 flex-wrap">
                        <div className="text-sm">
                          ...
                          <p>👤 {p.user_name || "?"} · 🗓️ {p.date_from} – {p.date_to} {(() => {
                            const running = p.activated && eq && eq.assigned_to === p.user_id;
                            if (running) return <span className="text-xs bg-green-100 text-green-800 px-2 py-0.5 rounded-full">{tx.planActive}</span>;
                            if (p.activated) return <span className="text-xs bg-slate-300 text-slate-700 px-2 py-0.5 rounded-full">{tx.planDone}</span>;
                            return <span className="text-xs bg-slate-200 text-slate-700 px-2 py-0.5 rounded-full">{tx.planPlanned}</span>;
                          })()}</p>
