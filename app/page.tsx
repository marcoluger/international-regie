...
export default function Home() {
  ...
  async function createReportFromInstruction(instruction: any, targetReport?: SavedReport | null) {
    ...
    if (targetReport) {
      ...
      if (isWeek) {
        // Wochen-Anweisung: nur in LEERE Tage der Woche schreiben (volle Tage nicht ueberschreiben).
        let wrote = 0;
        for (const date of instrDatesFor(instruction)) {
          const di = baseDays.findIndex((d) => d.date === date);
          if (di < 0 || dayHasContent(baseDays[di])) continue;
        ...
        if (wrote === 0) { setMessage(tx.dayFilled || "Dieser Tag ist im Regiebericht bereits ausgefüllt."); return; }
      } else {
        // Tages-Anweisung: genau der Zieltag. Ist er schon ausgefuellt -> abbrechen (kein Ueberschreiben).
        ...
        if (dayHasContent(baseDays[idx])) { setMessage(tx.dayFilled || "Dieser Tag ist im Regiebericht bereits ausgefüllt."); return; }
