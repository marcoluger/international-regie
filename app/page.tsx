...
async function loadWorkInstructions(companyId: string) {
  const { data, error } = await supabase
    ...
    .select(`
      *,
      work_instruction_tasks (*)
    `)
    .eq("company_id", companyId)
    .order("created_at", { ascending: false });

  if (error) {
    setMessage("Fehler beim Laden der Arbeitsanweisungen: " + error.message);
    return;
  }
...
async function saveWorkInstruction() {
  ...
  if (!currentCompany) {
    setMessage("Keine Firma geladen.");
    return;
  }

  if (!instructionTitle.trim()) {
    setMessage("Bitte Titel der Arbeitsanweisung eintragen.");
    return;
  }
  ...
  if (taskRows.length > 0) {
    ...
    if (taskError) {
      setMessage(
        "Arbeitsanweisung gespeichert, aber Schritte nicht: " +
          taskError.message
      );
      return;
    }
  ...
  setInstructionTitle("");
  setInstructionProject("");
  setInstructionCustomer("");
  setInstructionSite("");
  setInstructionDescription("");
  setInstructionTasks([""]);
  setInstructionProblems("");
...
async function updateWorkInstructionNote(id: string, note: string) {
  ...
  if (error) {
    setMessage("Fehler beim Speichern der Notiz: " + error.message);
    return;
  }
...
<section className="border rounded p-4 space-y-4 bg-white text-black">
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
    ...
    <input
      className="border p-3 text-black bg-white"
      placeholder="Kunde"
      value={instructionCustomer}
      onChange={(e) => setInstructionCustomer(e.target.value)}
    />
...
<input
  className="border p-3 w-full text-black bg-white"
  placeholder="Probleme / Hinweise"
  value={instructionProblems}
  onChange={(e) => setInstructionProblems(e.target.value)}
/>
  ...
  {instructionTasks.map((task, index) => (
    <input
      key={index}
      className="border p-3 w-full text-black bg-white"
      placeholder={`Arbeitsschritt ${index + 1}`}
      value={task}
      onChange={(e) => {
        const copy = [...instructionTasks];
        copy[index] = e.target.value;
        setInstructionTasks(copy);
      }}
    />
  ))}
  ...
  <button
    type="button"
    onClick={() => setInstructionTasks([...instructionTasks, ""])}
...
<section className="border rounded p-4 space-y-4 bg-white text-black">
  ...
  {workInstructions.length === 0 && (
    <p className="text-gray-600">Noch keine Arbeitsanweisungen vorhanden.</p>
  )}
...
{instruction.problems_text && (
  ...
  <input
    ...
    onChange={() =>
      toggleTaskCompleted(task.id, !(task.completed || false))
    }
  />

<textarea
  className="border p-3 w-full h-24 text-black bg-white"
  placeholder="Notiz / Rückmeldung Mitarbeiter"
  defaultValue={instruction.employee_note || ""}
...
<button
  ...
  onClick={() => {
    ...
    updateWorkInstructionNote(instruction.id, field.value);
  }}
  className="bg-gray-700 text-white px-3 py-2 rounded"
>
  Notiz speichern
  ...
  {(currentCompany?.role === "owner" ||
   currentCompany?.role === "project_manager" ||
  currentCompany?.role === "admin") && (
  <button
    type="button"
    onClick={() => deleteWorkInstruction(instruction.id)}
