...
export default function Home() {
  ...
  const instrTimesTimers = useRef<Record<string, any>>({});
  ...
  const [returnTarget, setReturnTarget] = useState<{ id: string; name: string; type?: string; startKm?: any; endSet?: boolean } | null>(null);
  const [returnNote, setReturnNote] = useState<string>("");
  const [returnKind, setReturnKind] = useState<"none" | "damage" | "other">("none");
  const [returnEndKm, setReturnEndKm] = useState<string>("");
  ...
  const [foremanUserIds, setForemanUserIds] = useState<string[]>([]);
  ...
  // Warnung, wenn der Regiebericht ungespeicherte Aenderungen hat (Seite verlassen/neu laden).
  useEffect(() => {
    if (typeof window === "undefined") return;
    const handler = (e: BeforeUnloadEvent) => { if (reportDirty) { e.preventDefault(); e.returnValue = ""; } };
    window.addEventListener("beforeunload", handler);
    return () => window.removeEventListener("beforeunload", handler);
  }, [reportDirty]);

  // Auto-Speichern eines bereits gespeicherten Berichts (leise, ~2 s nach der letzten Aenderung).
  useEffect(() => {
    if (!reportDirty || !currentReportId) return;
    const timer = setTimeout(() => { autosaveReport(); }, 2000);
    return () => clearTimeout(timer);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [reportDirty, currentReportId, days, employee, reportName, sigEmployee, sigCustomer]);
  ...
  async function createAbsence() {
    ...
    const end = absDraft.end || absDraft.start;
    // Validierung: Ende >= Start, nicht in der Vergangenheit, keine Ueberschneidung.
    if (end < start) { setMessage(tx.absEndBeforeStart || "Enddatum liegt vor dem Startdatum."); return; }
    const today = new Date().toISOString().slice(0, 10);
    if (start < today) { setMessage(tx.absPastDate || "Startdatum liegt in der Vergangenheit."); return; }
    const mine = (absences as any[]).filter((a) => a.user_id === user?.id && a.status !== "rejected" && a.start_date && a.end_date);
    const overlaps = mine.some((a) => !(end < a.start_date || start > a.end_date));
    if (overlaps) { setMessage(tx.absOverlap || "Für diesen Zeitraum gibt es bereits einen Antrag."); return; }
    setAbsSaving(true);
    try {
  ...
  function requestReturn(eq: any) {
    ...
    setReturnKind("none");
    setReturnEndKm("");
    setReturnTarget({ id: eq.id, name: eqTrans[eq.id] || eq.name || "", type: eq.type, startKm: eq.start_km, endSet: eq.end_km != null });
  }
  async function confirmReturn() {
    const tgt = returnTarget;
    if (!tgt) return;
    // Fahrzeug: End-km im gleichen Schritt erfassen (mit Plausibilitaetspruefung).
    if (tgt.type === "vehicle" && !tgt.endSet && returnEndKm.trim()) {
      const endN = Number(returnEndKm.replace(",", "."));
      const startN = Number(String(tgt.startKm ?? "").replace(",", "."));
      if (Number.isFinite(startN) && Number.isFinite(endN)) {
        if (endN < startN) { setMessage(tx.kmEndLow || "End-km kleiner als Start-km."); return; }
        if (endN - startN > 5000) { setMessage(tx.kmTooHigh || "km-Wert unplausibel."); return; }
      }
      await setKm(tgt.id, { endKm: returnEndKm.trim() });
    }
  ...
  function updateDay(index: number, field: keyof DayEntry, value: string) {
    const copy = [...days]; copy[index] = { ...copy[index], [field]: value }; setDays(copy); setReportDirty(true);
  }
  ...
  // Brutto-Dauer (Anfang→Ende) in Minuten, inkl. Schicht ueber Mitternacht.
  ...
  // Gesetzlicher Pausen-Vorschlag: >6 h → 30 min, >9 h → 45 min.
  function suggestBreak(start: string, end: string): string {
    const g = grossMinutes(start, end);
    if (g > 9 * 60) return "45";
    if (g > 6 * 60) return "30";
    return "";
  }
  // Warnung, wenn Ende <= Anfang (wird als Nachtschicht +24 h gerechnet).
  function timeLooksReversed(start?: string, end?: string): boolean {
  ...
  function updateDayTime(index: number, field: "startTime" | "endTime" | "breakMinutes", value: string) {
    ...
    // Pause automatisch vorschlagen, wenn noch keine eingetragen ist.
    if ((field === "startTime" || field === "endTime") && d.startTime && d.endTime && !(d.breakMinutes || "").trim()) {
      d.breakMinutes = suggestBreak(d.startTime, d.endTime);
    }
    const computed = computeDayHours(d.startTime, d.endTime, d.breakMinutes);
    if (computed !== "") d.hours = computed;
    copy[index] = d;
    setDays(copy); setReportDirty(true);
  }
  ...
  function updateExtra(dayIndex: number, exIndex: number, field: keyof DaySubEntry, value: string) {
    ...
    if ((field === "startTime" || field === "endTime") && sub.startTime && sub.endTime && !(sub.breakMinutes || "").trim()) {
      sub.breakMinutes = suggestBreak(sub.startTime, sub.endTime);
    }
  ...
  function setInstrTimeField(instrId: string, date: string, field: string, value: string) {
    setInstrTimes((prev) => {
      ...
      if ((field === "startTime" || field === "endTime") && day.startTime && day.endTime && !(day.breakMinutes || "").trim()) {
        day.breakMinutes = suggestBreak(day.startTime, day.endTime);
      }
      if (field === "startTime" || field === "endTime" || field === "breakMinutes") {
    ...
    // Automatisch speichern (leicht verzoegert), damit niemand "Zeiten speichern" vergisst.
    if (typeof window !== "undefined") {
      if (instrTimesTimers.current[instrId]) clearTimeout(instrTimesTimers.current[instrId]);
      instrTimesTimers.current[instrId] = setTimeout(() => {
        const inst = workInstructions.find((i: any) => i.id === instrId);
        if (inst) saveInstrTimes(inst, true);
      }, 1200);
    }
  }
  ...
  async function saveInstrTimes(instruction: any, quiet = false) {
    if (!user || !currentCompany) return;
    if (!quiet) setInstrTimesSaving(instruction.id);
    ...
    if (rows.length === 0) { if (!quiet) setInstrTimesSaving(""); return; }
  ...
  function startEditInstruction(instruction: any) {
    ...
    setForemanUserIds(instruction.foreman_user_ids || []);
  ...
  async function saveWorkInstruction() {
    ...
    if (editingInstructionId) {
      ...
      // Alte Schritte entfernen – werden gleich neu geschrieben (mit uebernommenen Kommentaren).
    ...
    // Vorhandene Schritte per Text zuordnen (erste noch nicht genutzte Uebereinstimmung),
    // um Kommentare/Notiz/Status/Fotos zu erhalten.
    ...
    const taskRows = instructionTasks
      ...
      .map((entry, sortOrder) => {
        const match = oldTasks.find((o: any) => !usedOld.has(o.id) && normTxt(o.task_text) === normTxt(entry.task));
        if (match) usedOld.add(match.id);
        return {
          work_instruction_id: instruction.id,
          task_text: entry.task,
          sort_order: sortOrder,
          photos: (instructionTaskPhotos[entry.originalIndex] && instructionTaskPhotos[entry.originalIndex].length ? instructionTaskPhotos[entry.originalIndex] : (match?.photos || [])),
          status: instructionTaskStatuses[entry.originalIndex] || match?.status || "open",
    ...
    setInstructionTitle(""); setInstructionProject(""); setInstructionCustomer(""); setInstructionSite(""); setInstructionStreet(""); setInstructionZip(""); setInstructionCity(""); setInstructionAddress2(""); setInstructionAddr2Desc(""); setInstructionAddr2Name(""); setInstructionAddr2Street(""); setInstructionAddr2Zip(""); setInstructionAddr2City(""); setAddr2Open(false); setInstructionDescription(""); setInstructionTasks([""]); setInstructionProblems(""); setInstructionMaterial(""); setInstructionWerkzeug(""); setInstructionPhotos([]); setInstructionTaskPhotos({}); setInstructionTaskStatuses({}); setAssignedUserIds([]); setForemanUserIds([]); setEditingInstructionId(null);
  ...
  // Leises Auto-Speichern des offenen Berichts (nur wenn bereits gespeichert).
  async function autosaveReport() {
    if (!user || !currentReportId) return;
    const name = reportName.trim() || buildReportName();
    const report = { report_name: name, employee, from_language: fromLanguage, to_language: toLanguage, pdf_language: pdfLanguage, days, project_id: selectedProjectId || null, signature_employee: sigEmployee || null, signature_customer: sigCustomer || null };
    const data = await reportsCall({ action: "save", id: currentReportId, report });
    if (!data?.error) setReportDirty(false);
  }
  ...
  function loadReport(report: SavedReport) {
    ...
    setReportVersion((v) => v + 1); setReportDirty(false);
  ...
  // Darf der aktuelle Benutzer in dieser Anweisung Kommentare/Status schreiben?
  // Manager (Owner/Admin/Projektleiter) immer; sonst nur als Bauleiter markierte Mitarbeiter.
  function canWriteInstruction(i: any): boolean {
    const role = currentCompany?.role;
    if (role === "owner" || role === "admin" || role === "project_manager") return true;
  ...
  async function createReportFromInstruction(instruction: any, targetReport?: SavedReport | null) {
    ...
    if (targetReport) {
      ...
      setDays(baseDays); setCurrentReportId(targetReport.id); setReportName(targetReport.report_name || ""); setReportLoaded(true); setReportVersion((v) => v + 1); setReportInstruction(instruction); setReportDirty(true); setActiveTab("regiebericht"); setMessage(t.msgReportPrepared);
      return;
    }
    ...
    setDays(copy); setCurrentReportId(null); setReportName(""); setReportLoaded(false); setReportVersion((v) => v + 1); setReportInstruction(instruction); setReportDirty(true); setActiveTab("regiebericht"); setMessage(t.msgReportPrepared);
  }
    ...
    <main className="max-w-5xl mx-auto p-4 md:p-8 space-y-6 bg-slate-100 min-h-screen text-black overflow-x-hidden">
        ...
        <div className="space-y-4">
          <section className="border border-slate-200 rounded-2xl p-4 shadow-sm space-y-4 bg-white text-black">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <input className="border p-3 text-black bg-white" placeholder={t.employee} value={employee} onChange={(e) => { setEmployee(e.target.value); setReportDirty(true); }} />
          ...
          {days.map((day, index) => (
            <section key={day.weekday} className="border border-slate-200 rounded-2xl p-4 shadow-sm space-y-3 bg-white text-black">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                {timeLooksReversed(day.startTime, day.endTime) && (<p className="md:col-span-2 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-2 py-1">⚠️ {tx.timeReversed}</p>)}
          ...
          {companyFeatures?.signature_enabled && (
            <section className="border border-slate-200 rounded-2xl p-4 shadow-sm space-y-3 bg-white text-black">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <SignaturePad label={pdfTexts[uiLanguage as keyof typeof pdfTexts]?.signatureEmployee || "Unterschrift Mitarbeiter"} value={sigEmployee} onChange={(v: string) => { setSigEmployee(v); setReportDirty(true); }} />
                <SignaturePad label={pdfTexts[uiLanguage as keyof typeof pdfTexts]?.signatureCustomer || "Unterschrift Kunde / Bauleitung"} value={sigCustomer} onChange={(v: string) => { setSigCustomer(v); setReportDirty(true); }} />
              </div>
            </section>
          )}
      ...
      {activeTab === "berichte" && (
        <section className="border border-slate-200 rounded-2xl p-4 shadow-sm space-y-4 bg-white text-black">
          ...
          <input className="border p-3 w-full text-black bg-white" placeholder={t.reportName} value={reportName} onChange={(e) => { setReportName(e.target.value); setReportDirty(true); }} />
                ...
                {companyUsers.filter(m => m.role === "employee").map((m) => (
                  <div key={m.user_id} className="flex items-center justify-between gap-2">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          if (e.target.checked) {
                            ...
                            setForemanUserIds(prev => prev.filter(id => id !== m.user_id));
                          }
                        }}
                        className="w-4 h-4"
                      />
                    ...
                    {assignedUserIds.includes(m.user_id) && (
                      <label className="flex items-center gap-1 cursor-pointer text-xs text-cyan-800" title={tx.foremanOnlyHint}>
                        <input
                          type="checkbox"
                          checked={foremanUserIds.includes(m.user_id)}
                          onChange={(e) => {
                            if (e.target.checked) setForemanUserIds(prev => [...prev, m.user_id]);
                            else setForemanUserIds(prev => prev.filter(id => id !== m.user_id));
                          }}
                          className="w-4 h-4"
                        />
                        👷 {tx.foreman}
            ...
            {!readOnlyUser && (
              <div className="flex gap-2 flex-wrap items-end border-b pb-3">
                ...
                <div className="flex flex-col">
                  ...
                  <input type="date" min={new Date().toISOString().slice(0, 10)} value={absDraft.start} onChange={(e) => setAbsDraft(p => ({ ...p, start: e.target.value }))} className="border p-2 rounded-lg text-black bg-white" />
                </div>
                <div className="flex flex-col">
                  <label className="text-xs text-gray-500">{t.absenceTo}</label>
                  <input type="date" min={absDraft.start || new Date().toISOString().slice(0, 10)} value={absDraft.end} onChange={(e) => setAbsDraft(p => ({ ...p, end: e.target.value }))} className="border p-2 rounded-lg text-black bg-white" />
                </div>
          ...
          {returnTarget && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => { setReturnTarget(null); setReturnNote(""); }}>
              <div className="bg-white rounded-2xl p-4 w-full max-w-md space-y-3 text-black shadow-xl" onClick={(ev) => ev.stopPropagation()}>
                ...
                {returnTarget.type === "vehicle" && !returnTarget.endSet && returnTarget.startKm != null && (
                  <div className="border-t pt-2">
                    <label className="text-xs text-gray-500 block mb-1">🚗 {tx.kmEnd} ({tx.kmStart}: {returnTarget.startKm})</label>
                    <input type="number" inputMode="numeric" value={returnEndKm} onChange={(ev) => setReturnEndKm(ev.target.value)} placeholder={tx.kmEnd} className="border p-2 rounded-lg w-full text-black bg-white" />
                    {returnEndKm.trim() && Number(returnEndKm.replace(",", ".")) < Number(String(returnTarget.startKm).replace(",", ".")) && (
                      <p className="text-xs text-red-600 mt-1">⚠️ {tx.kmEndLow}</p>
                    )}
                  </div>
                )}
                <div className="flex gap-2 justify-end">
                  <button type="button" onClick={() => { setReturnTarget(null); setReturnNote(""); setReturnKind("none"); setReturnEndKm(""); }} className="bg-gray-200 px-4 py-2 rounded-lg text-sm">{tx.planCancel}</button>
                  ...
                  {(instruction.work_instruction_tasks || []).sort((a: any, b: any) => a.sort_order - b.sort_order).map((task: any) => (
                    <li key={task.id} className="border border-slate-200 rounded-xl p-3 shadow-sm bg-gray-50 space-y-3">
                      <div className="flex items-center gap-2">
                        <select className="border rounded-lg p-1 text-sm text-black bg-white disabled:opacity-60" disabled={readOnlyUser || !canWriteInstruction(instruction)} value={task.status || "open"} onChange={(e) => updateTaskStatus(task.id, e.target.value)}>
                      ...
                      <div className="border-t pt-2 space-y-2">
                        {taskCommentList(task).filter((c: any) => chatOn || isMyComment(c)).map((c: any, ci: number) => (
                          <div key={c.id || ci} className={`border rounded-lg p-2 ${isMyComment(c) ? "bg-cyan-50 border-cyan-200" : "bg-gray-50"}`}>
                            <div className="flex items-start justify-between gap-2">
                              ...
                              {chatOn && isMyComment(c) && c.id && !readOnlyUser && canWriteInstruction(instruction) && (
                        ...
                        {(readOnlyUser || !canWriteInstruction(instruction)) ? (
                          <p className="text-xs text-gray-500">{readOnlyUser ? "👁️ " + t.readOnlyHint : "👷 " + tx.foremanOnlyHint}</p>
                        ) : (<>
                    ...
                    {instrDatesFor(instruction).map((date) => {
                      ...
                      return (
                        <div key={date} className="border border-slate-200 rounded-xl p-3 bg-gray-50 space-y-2">
                          ...
                          {timeLooksReversed(tm.startTime, tm.endTime) && (<p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-2 py-1">⚠️ {tx.timeReversed}</p>)}
                          <div className="space-y-2">
