        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50" onClick={() => setTransferConfirm(null)}>
          <div className="bg-white rounded-xl max-w-md w-full p-5 space-y-4 text-black" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="border rounded-lg p-2 bg-gray-50 text-sm">
              ...
              <span className="text-xs text-gray-500 block">{transferConfirm.instruction.work_date}</span>
            </div>
            <div className="flex flex-col gap-2">
              <button type="button" onClick={() => { const c = transferConfirm; setTransferConfirm(null); createReportFromInstruction(c.instruction, c.report); }} className="w-full bg-green-700 text-white px-4 py-3 rounded-lg">✅ {tx.exReportYes}</button>
              <button type="button" onClick={() => { const inst = transferConfirm.instruction; setTransferConfirm(null); setTransferInst(inst); }} className="w-full bg-gray-200 text-black px-4 py-2.5 rounded-lg">{tx.exReportNo}</button>
            </div>
          </div>
        </div>
      )}
